...
# #########
    ...
    function bond_res_Psi(vt, v_upper_thresh, # ttm,  # vmax,
                          sigmal,
                          r, gross_delta,
                          xi, k, _lambda, ugrid)
        resPsi_future = @spawn [_lambda * exp(-rdisc_pvs(r, xi, k, _lambda) * u) *
                                psi_v_td(vt, v_upper_thresh, u,
                                         sigmal, r, gross_delta) for u=ugrid]
    ...
    function bond_int_FPsi(vt, vbhl, # ttm, # vmax,
        ...
        cube_FPsi_future = @spawn [_lambda * exp(- rdisc_pvs(r, xi, k, _lambda) * u) *
                                   (exp(-rdisc(r, xi, k) * (ttm - u)) * (1 - bsm_F(v - log(vbhl),
    ...
    function bond_int_GPsi(vt, vbhl, #ttm, #  vmax,
        ...
        cube_GPsi_future = @spawn [_lambda * exp(- rdisc_pvs(r, xi, k, _lambda) * u) *
                                   bsm_G(v - log(vbhl), ttm - u,
                                         sigmal, r, gross_delta, xi, k) *
                                   (-dv_psi_v_td(vt, v, u, sigmal, r, gross_delta)) *
                                   (v - log(vbhl) > 0)
                                  for u=ugrid, v=vgrid]
