...
max_xval = .5 # maximum(svmdf[:, :sigmah])
svmdf = deepcopy(svmdf[abs.(svmdf[:, :lambda]) .<= max_xval, :])
...
if rerun_misrep | !(misrepdf_fn in readdir(script_dir))
    ...
    misrepdf[!, :s_sigmal] .= misrepdf[:, :sigmal]
    misrepdf[!, :s_sigmah] .= misrepdf[:, :sigmal]
    misrepdf[!, :r_sigmal] .= misrepdf[:, :sigmal]
    misrepdf[!, :s_obj_fun] .= firm_obj_fun
